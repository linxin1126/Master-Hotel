<template>
    <div id="orderDetail">
          ...
          <mu-col>
            <mu-card class="card" raised>
              ...
              <mu-card-actions class="actions">
                ...
                <mu-button color="error" @click="openDelete = true" v-show="order.orderStatus === -1">删除订单</mu-button>
      ...
      <mu-dialog title="删除订单" width="360" :open.sync="openDelete" :loading="deleteLoading">
        确认删除订单吗？
        <mu-button slot="actions" flat color="secondary" @click="openDelete = false">返回</mu-button>
        <mu-button slot="actions" flat color="primary" @click="deleteOrder()">确认删除</mu-button>
      </mu-dialog>
    </div>
</template>

<script>
  import {getOrderById, cancelOrder, payOrder, delOrder} from "@/api/order";
  ...
  export default {
      ...
      methods:{
        ...
        deleteOrder() {
          ...
          delOrder(this.order.orderId).then(res => {
            this.$router.push('/order')
            this.$toast.success("订单删除成功！")
            this.deleteLoading = false
          }).catch(err => {
              this.$toast.error("删除失败，原因："+ err.toString())
              this.cancelLoading = false
          })
        }
      }
