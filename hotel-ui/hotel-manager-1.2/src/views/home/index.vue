<template>
  <div id="home">
    <el-row :gutter="20">
      <el-col :span="8">
        <el-card class="box-card" shadow="hover">
          ...
          <el-button type="primary"  @click="sign()">入住登记</el-button>
          <el-button type="primary" @click="outsign()">退房结账</el-button>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card class="box-card" shadow="hover">
    ...
    <el-dialog title="入住房间号确认" :visible.sync="dialogCheckInVisible">
      <div>
        房间号：{{this.inRoomNumber}}
      </div>
      <div slot="footer" class="dialog-footer">
    ...
    <!-- 退房结账 -->
    <el-dialog title="退房结账" :visible.sync="dialogOutFormVisible">
      <el-form inline :model="form">
        <el-form-item label="入住房间号" >
          <el-input v-model="outRoomNumber" autoComplete="on"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="findCheckIn()">查询</el-button>
        </el-form-item>
      </el-form>
      <div v-if="isCheckShown && checkInDetail!=null">
        订单号：{{ checkInDetail.orderId }}<br>
        房间号：{{ checkInDetail.roomNumber }}<br>
        入住人：{{ checkInDetail.persons }}<br>
        入住时间：{{ checkInDetail.checkInTime | formatDay }}<br>
      </div>
      <div slot="footer" class="dialog-footer">
...
<script>
  ...
  import {checkIn, findCheckInByRoomNumber, checkOut} from "../../api/checkIn"
  export default {
  name: 'Home',
  data(){
    return{
  ...
  methods:{
    ...
    findOrder(){
      getOrderByNameAndPhone(this.form).then(res => {
        ...
        if (this.order == null) {
          ...
          this.checkForm.orderId = this.order.orderId;
          this.checkForm.persons = this.order.name;
          this.isOrderShown = true;
        }
      })
    },
    outsign(){
      this.dialogOutFormVisible = true;
    },
    findCheckIn(){
      findCheckInByRoomNumber(this.outRoomNumber).then(res => {
    ...
    checkIn(){
      checkIn(this.checkForm).then(res => {
        this.inRoomNumber = res.data.roomNumber
        if(res.code === 1000){
          this.$message({
            message: '入住成功！',
            type: 'success'
    ...
    cancelOut(){
      this.outRoomNumber = null
      this.dialogOutFormVisible = false
      this.checkInDetail = null
      this.isCheckShown = false
    },
    checkOut(){
      if(this.checkInDetail != null){
        checkOut(this.outRoomNumber).then(res => {
          if(res.code === 1000){
            this.$message({
              message: res.data,
              type: 'success'
